...
export function StatBlockForm({ aiEnabled = true }: Props) {
    ...
    const form = useForm<StatBlock>({
        ...
        validate: {
            ...
            challengeRating: (value) =>
                value < 0 || value > 30
                    ? "Challenge Rating must be between 0 and 30"
                    : null,
            type: (value) => (value.length < 1 ? "Type is required" : null),

            alignment: (value) =>
                value.length < 1 ? "Alignment is required" : null,
            armorClass: (value) =>
                value < 1 ? "Armor Class must be at least 1" : null,
            hitPoints: {
                value: (value) =>
                    value < 1 ? "Hit Points must be at least 1" : null,
                hitDice: (value) =>
                    value.length < 1 ? "Hit Dice is required" : null,
            },
            speed: (value) => {
    ...
    const addEntry = (setFn: any) =>
        setFn((prev: any) => [...prev, { name: "", desc: "" }]);
    const updateEntry = (
        setFn: any,
        index: number,
        field: string,
    ...
    const generate = async () => {
        ...
        try {
            ...
            showNotification({
                title: "Error",
                message:
                    "Failed to generate stat block. Please check your OpenAI API key and try again.",
                color: "red",
                autoClose: 5000,
            });
        } finally {
                        ...
                        <Stack gap={"md"}>
                            ...
                            {(edit || !aiEnabled) && (
                                <>
                                    ...
                                    <Button onClick={() => addEntry(setTraits)}>
                                        Add Trait
                                    </Button>
