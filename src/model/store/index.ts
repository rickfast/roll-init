...
export const store = {
    ...
    async load(): Promise<SaveData> {
        try {
            ...
            const data = await readTextFile('save.json', { baseDir: BaseDirectory.Home });
            const saveData = JSON.parse(data) as SaveData;

            console.log(`Loaded bestiary with ${Object.keys(saveData.bestiary).length} monsters and ${saveData.tracker?.combatants.length || 0} combatants`);
            return saveData;
        } catch (error) {
    ...
    async save(data: SaveData) {
        console.log('Saving bestiary to save.json', data);
        return writeTextFile('save.json', JSON.stringify(data, null, 2), { baseDir: BaseDirectory.Home })
